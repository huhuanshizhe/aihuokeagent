...
enrichRouter.post('/', async (req, res) => {
  try {
    ...
    for (const field of ['companyName', 'domain', 'country', 'industry'] as const) {
    ...
    if (req.body.candidateIds !== undefined && (!Array.isArray(req.body.candidateIds) || req.body.candidateIds.some((id: unknown) => typeof id !== 'string'))) {
      res.status(400).json({ success: false, error: 'candidateIds must be an array of strings' });
      return;
    }
    const candidateIds = req.body.candidateIds
      ? [...new Set((req.body.candidateIds as string[]).map(id => id.trim()).filter(Boolean))]
      : undefined;
