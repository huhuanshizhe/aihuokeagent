...
export async function generateKeywords(
  ...
  options: { mode: 'initial' | 'expansion'; existingKeywords?: string[]; maxKeywords?: number } = { mode: 'initial' }
  ...
  try {
    ...
    const parsed = await parseAIJson<Array<{ keyword: string; rationale?: string }>>(response.content);
...
function buildInitialPrompt(context: CompanyContext, targetCountries: string[]): string {
...
function buildExpansionPrompt(context: CompanyContext, existingKeywords: string[]): string {
