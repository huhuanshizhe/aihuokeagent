import type { Adapter, AdapterConfig, HealthStatus, NormalizedCandidate, SearchQuery, SearchResult } from './types.js';
...
export class BravePlacesAdapter implements Adapter {
  ...
  private async searchPlan(plan: BravePlacePlan, count: number): Promise<BravePlaceResult[]> {
    ...
    if (BRAVE_PLACE_COUNTRIES.has(plan.country)) params.set('country', plan.country);
    ...
    const data = await response.json() as { results?: BravePlaceResult[] };
    return data.results || [];
  }
...
export function buildBravePlacePlans(query: SearchQuery): BravePlacePlan[] {
...
function aggregateStats(stats: Array<{ keyword: string; fetched: number }>): Array<{ keyword: string; fetched: number }> {
  const result = new Map<string, number>();
  for (const stat of stats) result.set(stat.keyword, (result.get(stat.keyword) || 0) + stat.fetched);
