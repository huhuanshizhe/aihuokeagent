...
export function initSchema(): void {
  ...
  const columns = db.prepare('PRAGMA table_info(candidates)').all() as Array<{ name?: string }>;
  if (!columns.some(column => column.name === 'identity_key')) {
    db.exec('ALTER TABLE candidates ADD COLUMN identity_key TEXT');
  }
  const scanRunColumns = db.prepare('PRAGMA table_info(scan_runs)').all() as Array<{ name?: string }>;
  ...
  const enrichmentColumns = db.prepare('PRAGMA table_info(enrichments)').all() as Array<{ name?: string }>;
  ...
  for (const candidate of candidatesWithoutIdentity) {
    const identityKey = buildCandidateIdentity({
      ...
      candidateType: candidate.candidate_type as 'COMPANY' | 'OPPORTUNITY' | 'CONTACT',
      website: candidate.website as string | undefined,
      country: candidate.country as string | undefined,
    });
    if (identityKey) db.prepare('UPDATE candidates SET identity_key = ? WHERE id = ?').run(identityKey, candidate.id);
  }
  db.exec(`
