...
export interface GeoViewport { low: GeoPoint; high: GeoPoint }
...
export interface MarketPack {
  ...
  region: string;
  aliases: string[];
  languages: Array<{ code: string; name: string; priority: number }>;
  viewport: GeoViewport;
  clusters: MarketClusterResource[];
  localization: {
    phrases: Array<{ pattern: string; value: string; language: string }>;
  };
  sourceCodes: string[];
  updatedAt: string;
}
