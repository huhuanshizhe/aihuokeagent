...
export function buildCandidateIdentity(candidate: Pick<NormalizedCandidate, 'candidateType' | 'website' | 'displayName' | 'country'>): string | undefined {
  if (candidate.candidateType !== 'COMPANY') return undefined;
  ...
  if (name && country) return `name-country:${name}:${country}`;
  return undefined;
}

export function rankCandidateForEnrichment(candidate: NormalizedCandidate): CandidateRank {
  const reasons: string[] = [];
  let score = Math.max(0, Math.min(1, candidate.matchScore ?? 0.35)) * 0.55;

  if (candidate.website) { score += 0.16; reasons.push('has_website'); }
  if (candidate.email) { score += 0.08; reasons.push('has_email'); }
  if (candidate.phone) { score += 0.06; reasons.push('has_phone'); }
  if (candidate.description) { score += 0.05; reasons.push('has_description'); }
  if (candidate.country) { score += 0.04; reasons.push('has_country'); }
  if (candidate.industry) { score += 0.03; reasons.push('has_industry'); }
  if (candidate.isTargetCustomer) { score += 0.12; reasons.push('target_match'); }
...
export function selectCandidatesForEnrichment(candidates: NormalizedCandidate[], limit: number): CandidateSelection {
