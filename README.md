...
- Vercel 上 Express **本身就是 Serverless Function**（无法取消）；`includeFiles` 必须把 `src/ui/**`、`dist/ui/**`、`docs/**` 和 CSV 打进函数包，否则会出现「首页新、补全页仍是旧 HTML」这类混版
- 推送 Git 后若页面仍旧：到 Vercel → Deployments → 最新部署右侧 `⋯` → **Redeploy** 并勾选 **Clear cache / Use existing Build Cache 关掉**
