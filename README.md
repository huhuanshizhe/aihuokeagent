...
**部署注意：**
- `npm run build` 会把 CSV / UI / docs 拷到 `dist/`，自建只需部署 `dist` + `node_modules`（不必再单独拷源码 `data/`）
- UI 唯一真源是 `src/ui/`（无 `public/` 静态副本）；线上由 Express `express.static` 提供页面
- Vercel 上 Express **本身就是 Serverless Function**；`includeFiles` 必须带上 `src/ui/**`、`dist/ui/**`、`docs/**` 和 CSV，否则页面/OpenAPI 会 404
- 推送 Git 后若页面仍旧：到 Vercel → Deployments → 最新部署右侧 `⋯` → **Redeploy**，并关掉 Build Cache
- 源文件仍放在 `data/cache/thailand-factories.csv`（Git 真源）；下载页写入这里，再 `npm run build` / 提交
- 也可设 `THAI_FACTORY_CACHE_PATH` 指向外置路径
