...
**部署注意：**
- `npm run build` 会把 CSV 拷到 `dist/data/cache/`，自建只需部署 `dist` + `node_modules`（不必再单独拷源码 `data/`）
- Vercel 上 Express **本身就是 Serverless Function**（无法取消）；`includeFiles` 只是把 CSV 打进函数包，不是“多开一种部署模式”
