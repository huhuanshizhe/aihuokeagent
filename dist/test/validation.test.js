...
test('pipeline request rejects enrichment-only providers as discovery sources', () => {
    assert.throws(() => parsePipelineRequest({ keywords: ['automation'], adapters: ['hunter'] }), (error) => error instanceof RequestValidationError && error.details[0]?.includes('does not support discovery'));
});
test('pipeline request enforces bounded work limits', () => {
    assert.throws(() => parsePipelineRequest({ keywords: ['automation'], enrichTopN: 1000 }), RequestValidationError);
});
