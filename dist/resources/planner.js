...
export function buildDiscoveryResourcePlan(input) {
    ...
    const warnings = [];
    if (rawCountry && !market)
        warnings.push(`No active Market Pack exists for ${rawCountry}; global sources will be used`);
    if ((input.industry || input.keywords?.length) && !industry)
        warnings.push('No Industry Pack matched this intent; only supplied keywords will be used');
    ...
    const recommendedAdapters = unique(sources.filter(source => source.status === 'active').map(source => source.adapterCode).filter(Boolean));
    return {
        version: '1.0',
...
function scoreSource(source, marketSources, sourceTypes) {
    ...
    if (marketSources.includes(source.code)) {
        score += 0.15;
        reasons.push('market_pack_preferred');
    }
    if (sourceTypes.includes(source.sourceType)) {
        score += 0.1;
        reasons.push('industry_source_type_match');
    }
    ...
    if (!source.requiresApiKey) {
        score += 0.03;
        reasons.push('no_api_key_required');
    }
