...
export class AISearchAdapter {
    ...
    applySearchConstraints(rawQuery, countries, exclusions) {
        const parts = [rawQuery.trim()];
        for (const country of countries) {
            if (country && !rawQuery.toLowerCase().includes(country.toLowerCase()))
                parts.push(country);
        }
        for (const exclusion of exclusions.slice(0, 8)) {
            const safe = exclusion.trim().replace(/["']/g, '');
            if (safe)
                parts.push(safe.includes(' ') ? `-"${safe}"` : `-${safe}`);
        }
        return parts.filter(Boolean).join(' ');
    }
    async executeSearches(queries, _originalQuery) {
        const allResults = [];
        const warnings = [];
        ...
        const resultsArrays = await mapWithConcurrency(queries, 2, async (q) => {
            ...
            if (config.serpapi.apiKey) {
                try {
                    ...
                    if (q.region)
                        params.set('gl', q.region.toLowerCase());
                    ...
                    if (res.ok) {
                        ...
                        warnings.push(`${q.sourceKeyword}: SerpAPI HTTP ${res.status}`);
                }
                catch (e) {
    ...
    async parseResults(results, query) {
        if (results.length === 0)
...
排除词: ${query.excludeKeywords?.join(', ') || '(无)'}`;
        try {
            const result = await chatCompletion([{ role: 'system', content: systemPrompt }, { role: 'user', content: userPrompt }], { temperature: 0.2, maxTokens: 2048 });
            const parsed = await parseAIJson(result.content);
    ...
    parseFallbackCompanies(results, query) {
        const blockedHosts = [
        ...
        const targetNames = targetCodes.map(code => getCountryDisplayName(code)?.toLowerCase()).filter(Boolean);
        ...
        for (const item of results) {
            ...
            if (seenHosts.has(host) || blockedHosts.some(blocked => host === blocked || host.endsWith(`.${blocked}`)))
                continue;
            if (documentPattern.test(`${item.title} ${item.snippet}`) || sourcePagePattern.test(`${item.title} ${item.snippet}`))
                continue;
            ...
            if (candidates.length >= (query.maxResults || 10))
                break;
        }
        return candidates;
    }
