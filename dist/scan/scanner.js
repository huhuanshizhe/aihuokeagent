  ...
  `).run(runId, JSON.stringify(options.keywords || []), JSON.stringify(options.countries || []), options.industry || null, new Date().toISOString());
    try {
        ...
        if (keywords.length === 0 && options.companyName) {
            ...
            const generated = await generateKeywords({ companyName: options.companyName, companyIntro: options.companyIntro, products: options.products, targetIndustries: options.industry ? [options.industry] : undefined }, options.countries || [], { mode: 'initial' });
        ...
        const warnings = Object.entries(adapterResults).flatMap(([adapter, result]) => result.warnings.map(warning => `[${adapter}] ${warning}`));
        db.prepare(`
      ...
      WHERE id = ?
    `).run(totalFetched, totalFound, totalNew, totalRejected, totalQualified, totalReview, totalDeferred, JSON.stringify(errors), JSON.stringify({ resourcePlan, adapterResults, warnings, rejectedSamples, reviewSamples }), new Date().toISOString(), runId);
...
// ==================== 候选入库 ====================
function upsertCandidate(runId, adapterCode, item) {
    const id = randomUUID();
    ...
    try {
      ...
      LIMIT 1
    `).get(adapterCode, externalId, identityKey || null, identityKey || null, adapterCode, externalId);
        if (existing) {
            db.prepare(`
        ...
        WHERE id = ?
      `).run(item.description || null, item.website || null, item.phone || null, item.email || null, item.address || null, item.country || null, item.city || null, item.industry || null, item.businessType || null, item.products ? JSON.stringify(item.products) : null, item.brands ? JSON.stringify(item.brands) : null, item.employeesCount || null, item.isTargetCustomer ? 1 : 0, item.targetReason || null, item.matchScore ?? null, item.qualificationTier || null, item.matchScore ?? null, item.qualificationReasons ? JSON.stringify(item.qualificationReasons) : null, item.matchScore ?? null, item.matchExplain ? JSON.stringify(item.matchExplain) : null, identityKey || null, existing.id);
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, runId, adapterCode, externalId, item.displayName, item.candidateType, item.description || null, item.website || null, item.phone || null, item.email || null, item.address || null, item.country || null, item.city || null, item.industry || null, item.businessType || null, item.products ? JSON.stringify(item.products) : null, item.brands ? JSON.stringify(item.brands) : null, item.employeesCount || null, item.isTargetCustomer ? 1 : 0, item.targetReason || null, item.qualificationTier || null, item.qualificationReasons ? JSON.stringify(item.qualificationReasons) : null, item.matchScore || null, item.matchExplain ? JSON.stringify(item.matchExplain) : null, identityKey || null, item.rawData ? JSON.stringify(item.rawData) : null, item.sourceUrl || null);
        if (result.changes > 0) {
...
function linkCandidateToRun(runId, candidateId, adapterCode) {
    db.prepare(`
    INSERT OR IGNORE INTO scan_run_candidates (run_id, candidate_id, adapter_code)
    VALUES (?, ?, ?)
  `).run(runId, candidateId, adapterCode);
}
function saveCandidateSource(candidateId, adapterCode, externalId, item) {
    db.prepare(`
    INSERT OR REPLACE INTO candidate_sources
      (candidate_id, adapter_code, external_id, source_url, raw_data, discovered_at)
    VALUES (?, ?, ?, ?, ?, datetime('now'))
  `).run(candidateId, adapterCode, externalId, item.sourceUrl || null, item.rawData ? JSON.stringify(item.rawData) : null);
}
// ==================== 查询接口 ====================
export function getScanResults(runId) {
  ...
  `).all(runId);
    ...
    return uniqueRows.map(row => ({
        ...
        candidateType: row.candidate_type || 'COMPANY',
        ...
        isTargetCustomer: row.is_target_customer === 1,
        targetReason: row.target_reason,
        qualificationTier: row.qualification_tier,
        qualificationReasons: row.qualification_reasons ? JSON.parse(row.qualification_reasons) : undefined,
        matchScore: row.match_score,
        matchExplain: row.match_explain ? JSON.parse(row.match_explain) : undefined,
        rawData: row.raw_data ? JSON.parse(row.raw_data) : undefined,
    }));
}
export function getScanRun(runId) {
    return db.prepare('SELECT * FROM scan_runs WHERE id = ?').get(runId);
}
export function listScanRuns(limit = 20) {
