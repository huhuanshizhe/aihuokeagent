...
function hasCountryEvidence(candidate, query) {
    ...
    if (candidateCode && targetCodes.includes(candidateCode))
        return true;
...
export function qualifyDiscoveredCandidate(candidate, query) {
    const reasons = [];
    ...
    if (keywordGroups.length === 0 && query.industry)
        keywordGroups.push({ keyword: query.industry, tokens: tokens([query.industry]) });
    ...
    if (candidate.website && !isSocialHost) {
        score += 0.12;
        reasons.push('has_website');
    }
    if (candidate.website && isSocialHost) {
        score += 0.03;
        reasons.push('social_web_presence_only');
    }
    if (candidate.phone) {
        score += 0.05;
    ...
    const sourceMatchedKeywords = Array.isArray(candidate.rawData?.sourceMatchedKeywords)
        ? candidate.rawData.sourceMatchedKeywords.filter((value) => typeof value === 'string')
        : [];
    ...
    if (host && NON_LEAD_HOSTS.some(blocked => host === blocked || host.endsWith(`.${blocked}`))) {
        if (source === 'ai_search' || !isSocialHost)
            rejectionReasons.push(`non_lead_domain:${host}`);
        else
            reasons.push(`weak_social_domain:${host}`);
    }
    ...
    if (negativeMatches.length)
        rejectionReasons.push(`negative_keyword:${negativeMatches.slice(0, 3).join(',')}`);
    const placeTypes = Array.isArray(candidate.rawData?.types)
        ? candidate.rawData.types.filter((type) => typeof type === 'string')
        : [];
    for (const type of placeTypes) {
        if (!GENERIC_CONSUMER_TYPES.has(type))
            continue;
