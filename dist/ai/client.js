...
// ==================== SSE 解析 ====================
function parseSSEResponse(sseText, fallbackModel) {
    let fullContent = '';
...
// ==================== curl 执行器 ====================
function execCurl(args, timeoutMs) {
...
// ==================== 调用实现 ====================
async function callViaCurl(apiKey, baseUrl, messages, model, options) {
...
async function callViaFetch(apiKey, baseUrl, messages, model, options) {
    const { temperature = 0.3, maxTokens = 4096, topP = 0.8, timeout = 300 } = options;
...
export async function parseAIJson(content) {
    ...
    try {
        ...
        try {
            ...
            return JSON.parse(stripJsonFences(repaired.content));
        }
        catch {
